...
class MechanicPage extends StatefulWidget {
  const MechanicPage({super.key, required this.position});
...
class _MechanicPageState extends State<MechanicPage> {
  ...
  getmechanices() async {
    List<LatLng> list = await API.getNearbyMechanics(
  ...
  showPinsOnMap() async {
    for (var mechanic in mechanics.asMap().entries) {
      setState(() {
        _markers.add(Marker(
          markerId: MarkerId('driver${mechanic.key}'),
          position: mechanic.value,
        ));
      });
    }
  }
