...
class CarwashPage extends StatefulWidget {
  const CarwashPage({super.key, required this.position});
...
class _CarwashPageState extends State<CarwashPage> {
  ...
  getCarWashes() async {
    List<LatLng> list = await API.getNearbyCarwashes(
  ...
  showPinsOnMap() async {
    for (var carwash in carwashes.asMap().entries) {
      setState(() {
        _markers.add(Marker(
          markerId: MarkerId('driver${carwash.key}'),
          position: carwash.value,
        ));
      });
    }
  }
